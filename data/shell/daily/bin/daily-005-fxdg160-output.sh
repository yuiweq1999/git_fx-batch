...
# 環境變數
JOB_NAME="daily-005-fxdir00-output"
export JOB_NAME
...
s_ir_iran_output() {
    ...
    "${STEP_DIR}"/daily-005-fxdir00-job.sh "????"
    local status=${?}
...
# ==============
x_xxxx_output() {
    echo "x_xxxx_output start ${@}"
    export OUTPUT_FILE="$OUTPUT_DIR"/CUSTOM.NEW.FILES
    "${STEP_DIR}"/daily-005-fxdir00-job.sh "????"
    local status=${?}
    echo "x_xxxx_output result code:${status}"
    check_status_and_exit ${status}
}
...
fxdir00_common_main() {
    ...
    if [ $# -eq 0 ]; then
        ...
        "1" | "s_ir_iran_output")
            s_ir_iran_output
            ;;
        "2" | "s_ir_iran_other_output")
            s_ir_iran_other_output
            ;;
        "3" | "s_pt_ir_stbcf_output")
            s_pt_ir_stbcf_output
            ;;
        "4" | "s_pt_ir_stbjm1_output")
            s_pt_ir_stbjm1_output
            ;;
        "5" | "s_ir_aa_output")
            s_ir_aa_output
        ...
        "7" | "x_xxxx_output")
            x_xxxx_output
            ;;
        "8" | "s_ir_accsub_output")
            s_ir_accsub_output
            ;;
        "9" | "s_ir_accmain_a87d_output")
            s_ir_accmain_a87d_output
            ;;
        "10" | "s_ir_accsub_obu_output")
            s_ir_accsub_obu_output
            ;;
        "11" | "s_ir_accmain_a87d_obu_output")
            s_ir_accmain_a87d_obu_output
            ;;
        "12" | "s_ir_z6_day_output")
            s_ir_z6_day_output
            ;;
        *)
            echo "unknown: ${arg}"
